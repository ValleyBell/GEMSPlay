...
static SN76489_Context SN76489Data[MAX_CHIPS];
...
SN76489_Context* SN76489_Init( int PSGClockValue, int SamplingRate)
{
	...
	if(chip)
	{
		...
		for( i = 0; i <= 3; i++ )
			chip->panning[i][0] = chip->panning[i][1] = 1.0f;
		SN76489_Reset(chip);
	}
	return chip;
}
...
void SN76489_Update(SN76489_Context* chip, INT16 *buffer, int length)
{
	...
	for( j = 0; j < length; j++ )
	{
		...
		if ( chip->Mute >> 3 & 1 )
		{
			//chip->Channels[3] = PSGVolumeValues[chip->Registers[7]] * ( chip->NoiseShiftRegister & 0x1 ) * 2; /* double noise volume */
			// Now the noise is bipolar, too. -Valley Bell
			chip->Channels[3] = PSGVolumeValues[chip->Registers[7]] * (( chip->NoiseShiftRegister & 0x1 ) * 2 - 1);
		}
		else
			chip->Channels[i] = 0;
